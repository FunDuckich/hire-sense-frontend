        ...
        <>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                {applications.length > 0 ? (
                  ...
                  <span
                      className={`px-3 py-1 text-xs font-semibold rounded-full ${statusMap[app.status]?.class || 'bg-gray-100 text-gray-800'}`}>
                    {statusMap[app.status]?.text || app.status}
                                    ...
                                    {app.status === 'COMPLETED' && (
                                        <Link to={`/my-feedback/${app.id}`}
                                              className="text-sm bg-green-600 text-white py-1 px-3 rounded-lg hover:bg-green-700 font-semibold">
                                            Посмотреть итоги
                                        </Link>
                                    )}
                                </div>
